...
### Protocol V2
Second major version of Andamio Protocol smart contracts, [independently audited](/docs/security-audit) by TxPipe. Improvements in scalability, security, and features.
...
Feature allowing organizations to pay blockchain transaction fees on behalf of users, removing Web3 adoption barriers. Available to enterprise API keys — see [Sponsored Transactions](/docs/api/core/sponsored-transactions).
...
The act of notifying the Andamio gateway that a transaction has been submitted to the Cardano network. After `wallet.submitTx()` returns a `txHash`, the app POSTs it to `/api/v2/tx/register` along with the transaction type and any metadata. Before this POST, the gateway has no record of the transaction. After it, the gateway creates a tracking entry, adds the transaction to its confirmation queue, and opens an SSE stream so the frontend can receive state updates in real time.

### Server-Sent Events (SSE)
A web technology where the server keeps an HTTP connection open and pushes updates to the browser in real time — one direction only (server to browser). Used by the Andamio gateway to stream transaction state changes (`pending → confirmed → updated`) to the app as they happen, without the browser needing to repeatedly ask for updates. If the SSE connection fails, the app falls back to polling.

### Module-level State
State that lives outside any React component — created once when a JavaScript file is first imported and kept alive for the entire browser session, regardless of page navigation. The Andamio transaction watcher store uses module-level state so that SSE connections and polling continue running in the background even when a user navigates away from the page where they submitted a transaction.

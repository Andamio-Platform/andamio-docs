...
A decentralized identity standard built on Cardano that enables users to maintain complete control over their professional identity and credentials. SSOI uses a minimalist design - storing just a user's chosen alias and references to their various local states - which creates maximum flexibility for different applications to integrate while maintaining interoperability. In Andamio V2, the SSOI data is stored on-chain in the Access Token datum, and only the owner of the wallet holding the Access Token can authorize transactions that update their identity. SSOI establishes the pattern for decentralized identity on Cardano - a minimal protocol that enables maximum innovation on top. Andamio is working to establish SSOI as a Cardano Improvement Proposal (CIP) standard to enable widespread adoption across the ecosystem.

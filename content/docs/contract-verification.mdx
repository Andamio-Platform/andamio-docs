...
This page proves **which code is deployed** (identity). For **whether that code is correct** (behavior), see the independent [Security Audit](/docs/security-audit).
...
- **Correctness** ([Security Audit](/docs/security-audit)): TxPipe independently audited the validator and minting-policy surface, covering threats like token theft, protocol halting, and double satisfaction.

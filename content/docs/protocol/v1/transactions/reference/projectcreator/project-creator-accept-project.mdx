---
title: "ProjectCreatorAcceptProject"
description: "Project creator accepts a contributor's project proposal or application"
---

# ProjectCreatorAcceptProject

Project creator accepts a contributor's project proposal or application

## Overview

This transaction is performed by **ProjectCreator** users and falls under the **Project Management** category.

## Prerequisites

### Required Tokens
The following tokens are required to execute this transaction:

- **100at**
- **projectCreatorNFT**

## Transaction Details

### Estimated Fee
**3.0 ADA**

### Transaction Type
- **Role:** ProjectCreator
- **Category:** Project Management
- **Multi-Signature:** No

## Transaction Structure

This transaction includes:

### Inputs
The transaction consumes the following UTxOs:
- User wallet UTxO
- Protocol state UTxOs
- Authorization tokens

### Reference Inputs
The transaction references:
- Parameter reference scripts
- Instance reference data
- Global state information

### Outputs
The transaction produces:
- Updated state UTxOs
- Modified user wallet
- Any newly minted tokens

### Transaction Flow

![Transaction Diagram](/diagrams/protocol/project-creator-accept-project.png)

## API Reference

### Endpoint
```
POST /tx/v0/projectcreator/accept-project
```

### Request Example
```json
{
  "userAlias": "string",
  "additionalParams": {
    // Transaction-specific parameters
  }
}
```

### Response Example
```json
{
  "success": true,
  "transactionId": "tx_hash",
  "fee": "3.0 ADA"
}
```

## Code Examples

### JavaScript/TypeScript
```typescript
import { AndamioAPI } from '@andamio/api';

const api = new AndamioAPI();

const result = await api.transactions.projectCreatorAcceptProject({
  userAlias: "your-alias",
  // Additional parameters
});
```

### Python
```python
from andamio_api import AndamioAPI

api = AndamioAPI()

result = api.transactions.project_creator_accept_project(
    user_alias="your-alias",
    # Additional parameters
)
```

## Error Handling

Common errors that may occur:

| Error Code | Description | Solution |
|------------|-------------|----------|
| `INSUFFICIENT_TOKENS` | Required tokens not available | Ensure wallet contains required tokens |
| `INVALID_SIGNATURE` | Signature validation failed | Check signing process and permissions |
| `TRANSACTION_FAILED` | Transaction execution failed | Review transaction parameters and retry |

## Related Transactions

- [ProjectCreatorRefuseProject](./project-creator-refuse-project.mdx)
- [ProjectCreatorDenyProject](./project-creator-deny-project.mdx)

## See Also

- [All ProjectCreator Transactions](../index.mdx#projectcreator-transactions)
- [Transaction Overview](../../index.mdx)
- [API Documentation](../../apis/reference/)
- [Getting Started Guide](../../getting-started.mdx)

...
### Token Name Prefixes
- `u` - User access tokens (e.g., `<access_token_policyid>.u<alias>`)
- `g` - Global state tokens (e.g., `<access_token_policyid>.g<alias>`)
...
2. Dynamic values appear in contexts like:
   ```yaml
   value:
     - "1 <course_nft_policyid>.<course_id>"
   datum:
     courseId: "<course_id>"
   ```
...
1. Define patterns with placeholders:
   ...
   tokens:
     access-token-user:
       asset-id: "<access_token_policyid>.u<alias>"
   ```

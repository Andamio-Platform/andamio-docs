...
`andamio-dev` is a public [Agent Skills](https://agent-skills.cc) package, originally built as a Claude Code plugin, that pairs with the [Andamio CLI](/docs/apps-tooling/cli) so an AI coding agent can operate Andamio alongside you. It serves two purposes:
...
<Callout type="info" title="Canonical source: the andamio-dev repo">
  `andamio-dev` is open source and the repo is the source of truth for the full
  skill set, course content, and reference material:
  [github.com/Andamio-Platform/andamio-dev](https://github.com/Andamio-Platform/andamio-dev).
</Callout>

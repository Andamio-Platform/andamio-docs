...
- [User authentication](/docs/api/guides/cli/authentication) (wallet login) for `tx build` and `tx register`
...
For hash verification and diagnostics (compute-hash, verify-hash for both courses and tasks), see [Hash Verification](/docs/api/guides/cli/hash-verification).

...
- [User authentication](/docs/apps-tooling/cli/authentication) (wallet login) for `tx build` and `tx register`
...
For hash verification and diagnostics (compute-hash, verify-hash for both courses and tasks), see [Hash Verification](/docs/apps-tooling/cli/hash-verification).
...
States: `pending` → `confirmed` → `updated` (or `failed` / `expired`). See [Error Handling](/docs/api/guides/error-handling) for recovery actions per state.

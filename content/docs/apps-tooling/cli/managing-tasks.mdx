...
If you own projects (created via on-chain transaction), manage them with the `project owner` subgroup. All require [user authentication](/docs/apps-tooling/cli/authentication).
...
All task commands require [user authentication](/docs/apps-tooling/cli/authentication) (wallet login) and manager access on the project.
...
Managers review contributor commitments on their projects. Requires [user authentication](/docs/apps-tooling/cli/authentication) and manager access.
...
Contributors commit to tasks, submit evidence, and track their work. All `project contributor` commands require [user authentication](/docs/apps-tooling/cli/authentication).
...
For on-chain operations (task commitment, credential claim), use `tx run` with the appropriate transaction endpoint. See [Transaction Signing](/docs/apps-tooling/cli/transaction-signing) for the full workflow.

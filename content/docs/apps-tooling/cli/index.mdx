...
| List submissions awaiting review | `andamio teacher assignments list --course <id>` |
| Read one submission | `andamio teacher assignments get <course-id> <module-code> <student-alias>` |
| Build an assessment without signing it | `andamio teacher assessment build --course-id <id> --alias <you> --decision <student>=accept` |

`teacher assignments` returns each submission as Markdown in `content.evidence_text`, alongside the raw Tiptap document in `content.evidence` — read the first for the prose, the second to verify a hash.

`teacher assessment build` stops at the unsigned transaction so a person can review the decisions before approving them. Signing and submitting stay separate steps (see [Transactions](#transactions)); `andamio tx run` still does the whole lifecycle in one go.

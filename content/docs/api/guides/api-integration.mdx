...
| `Authorization: Bearer <jwt>` | User-level (wallet signature) | [Authentication](/docs/api/guides/authentication) |
...
|----------|-----------|-------------|-------|
| User Auth | `/api/v2/auth/login/*` | Wallet challenge/verify, User JWT | [Authentication](/docs/api/guides/authentication) |
| Developer Auth | `/api/v2/auth/developer/*` | Registration, email verification, Developer JWT | [Developer Accounts](/docs/api/guides/developer-accounts) |
| API Keys | `/api/v2/apikey/*` | Key request, rotation, profile, usage | [API Keys](/docs/api/guides/api-keys) |
| Billing | `/api/v2/billing/*` | Plans, checkout, subscription management | [Billing](/docs/api/guides/billing) |
| Verification | `/api/v2/verify/*` | Access token ownership proof | [Verification](/docs/api/guides/access-token-verification) |
...
| Transactions | `/api/v2/tx/*` | Build, register, status, SSE stream (17 build endpoints) | [Transactions](/docs/api/guides/transactions) |
| Users | `/api/v2/users/*` | User state, owned/enrolled/completed resources | |
| Events | `/api/v2/events/*` | Parse what happened in a confirmed transaction | [API Concepts](/docs/api/guides/api-concepts) |
...
For transaction building and submission, see [Transaction Handling](/docs/api/guides/transactions).
...
See [Error Handling](/docs/api/guides/error-handling) for recovery patterns.

## Next Steps

- [API Concepts](/docs/api/guides/api-concepts) — Source field, safe refetch timing, event queries
- [Transaction Handling](/docs/api/guides/transactions) — Build, sign, and submit transactions
- [Error Handling](/docs/api/guides/error-handling) — Recovery patterns and failure modes

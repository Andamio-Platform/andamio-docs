...
| **User JWT** | Action authority — "who's acting" | User signs a wallet challenge ([Authentication](/docs/api/guides/authentication)) | Requests that act on behalf of a user (`Authorization: Bearer` header) |
| **Developer JWT** | Account management | Developer email/password login (see below) | API key creation, rotation, billing |
| **Attestation JWT** | Offline proof — "prove it to a third party" | Returned by the [access token verification](/docs/api/guides/access-token-verification) flow | Verify Andamio identity without API access |
...
Developers can also authenticate with a CIP-30 wallet signature instead of a password, using the same challenge/sign/verify pattern as [user wallet login](/docs/api/guides/authentication). This issues a Developer JWT without a stored password.
...
Attestation JWTs are returned by the [Access Token Verification](/docs/api/guides/access-token-verification) flow.
...
- [Authentication](/docs/api/guides/authentication) — Wallet-based user login for end users
- [Access Token Verification](/docs/api/guides/access-token-verification) — Verify identity for third parties

...
With just the API key you can read public data — no app template, no frontend, just curl.
...
{
  "courses": [
    { "course_id": "dbd4f125...406bcac3", "title": "Andamio for Contributors", "modules": 4, "status": "active" }
  ]
}
```
...
```bash
curl -X GET "https://preprod.api.andamio.io/api/v2/course/<course_id>/modules" \
  -H "X-API-Key: YOUR_API_KEY"
```
...
These are read-only, public endpoints. For user-specific actions — enrolling, submitting assignments, claiming credentials — you authenticate the user with a wallet signature. See [Authentication](/docs/api/accounts/authentication) for the full JWT flow.

...
### Don't Refetch on `confirmed`

The most common mistake is treating `confirmed` as the terminal state:

```typescript
// ❌ WRONG — data will be stale
if (status.state === "confirmed") {
  refetchData(); // DB hasn't synced yet!
}

// ✅ CORRECT — wait for DB sync
if (status.state === "updated") {
  refetchData(); // Now it's safe
}
```

`confirmed` means "found on-chain" but the gateway is still syncing to the database. Wait for `updated` (~30s after on-chain confirmation) before refetching.

## Using the App Template Hooks

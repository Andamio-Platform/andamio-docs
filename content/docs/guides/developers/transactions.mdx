...
You build the UX, Andamio handles credentials. Every transaction follows a consistent lifecycle across all transaction types.
...
The flow: your app requests a transaction → user signs → Andamio submits, confirms, and syncs to the database. Your app gets notified when it's done.

> **Deep dive**: For state transitions, error handling patterns, and architecture details, see [Transaction State Machine](/docs/protocol/v2/state-machine).

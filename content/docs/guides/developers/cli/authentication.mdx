...
- The `--alias` flag is required — this is your Andamio username
...
# Courses where you're a teacher (can import/export content)
andamio teacher courses

# Projects where you're a manager (can create/manage tasks)
...
**`forbidden` errors** — You may not be a teacher on the course you're trying to modify. Check with `andamio teacher courses` to see which courses you have write access to. For task operations, check `andamio manager projects`.

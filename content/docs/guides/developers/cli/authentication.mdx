...
**`forbidden` errors** — You may not have the required role. Check with `andamio teacher courses` for teacher access, `andamio manager projects` for manager access, `andamio course student courses` for student enrollment, or `andamio project contributor list` for contributor access.

...
All four commands require `--course-id` and `--module-code`. The `update-module-status` command also requires `--status`.
...
### On-Chain Commitment with Evidence

For the full on-chain flow (build, sign, submit, register), use `commit-tx`. This combines evidence preparation with the Cardano transaction lifecycle in a single command:
...
# For chain-only modules, use --slt-hash instead of --module-code
andamio course student commit-tx \
  --course-id <id> --slt-hash 77547a... \
  --evidence-file ./my-evidence.md \
  --skey ./payment.skey
```

The CLI converts your markdown evidence to Tiptap JSON, computes its Blake2b-256 hash, builds the on-chain transaction, signs it locally, submits to the network, and stores the evidence off-chain — all in one step.

Evidence is optional — omit `--evidence`/`--evidence-file` to commit on-chain without evidence. Use `--slt-hash` instead of `--module-code` for chain-only modules that don't have a module code.

See [Transaction Signing](/docs/guides/developers/cli/transaction-signing#domain-specific-transaction-commands) for more details.

...
List pending reviews, then accept or refuse:
...
The `--decision` flag accepts `accept` or `refuse`. Required flags: `--course-id`, `--module-code`, `--participant-alias`, `--decision`.
...
# List pending commitments with module code and participant
andamio course teacher commitments --course-id "$COURSE_ID" -o json | \
  jq -r '.data[] | "\(.content.course_module_code) \(.content.participant_alias)"'

# Review a specific commitment
andamio course teacher review --course-id "$COURSE_ID" --module-code 101 --participant-alias student1 --decision accept
```

To batch-accept (use with caution — review each submission first in production):

```bash
andamio course teacher commitments --course-id "$COURSE_ID" -o json | \
  jq -r '.data[] | "\(.content.course_module_code) \(.content.participant_alias)"' | \
  while read MODULE ALIAS; do
    andamio course teacher review --course-id "$COURSE_ID" --module-code "$MODULE" --participant-alias "$ALIAS" --decision accept
  done
```

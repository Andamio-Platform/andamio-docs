...
## Domain-Specific Transaction Commands

For common on-chain operations, the CLI provides high-level commands that handle evidence preparation, hash resolution, and the full transaction lifecycle automatically.

### Assignment Commitment (Course Student)

Commit to a course assignment on-chain, optionally including evidence:
...
This single command:
1. Converts markdown evidence to Tiptap JSON and computes its Blake2b-256 hash
2. Resolves the `slt_hash` from the module code
3. Builds the on-chain transaction with the evidence hash as `assignment_info`
4. Signs, submits, registers, and polls for confirmation
5. Submits the evidence to the off-chain API with the `pending_tx_hash`

Evidence is optional — omit `--evidence`/`--evidence-file` for a commit-only transaction (enroll on-chain without evidence).

For chain-only modules (no module code), use `--slt-hash` instead of `--module-code`:

```bash
andamio course student commit-tx \
  --course-id <id> --slt-hash 77547ab0... \
  --evidence-file ./my-evidence.md \
  --skey ./payment.skey
```

### Task Commitment (Project Contributor)

Commit to a project task on-chain, optionally including evidence:
...
Same lifecycle as the course command, but resolves `task_hash` and `contributor_state_id` automatically.

Both commands support `--no-wait`, `--timeout`, `--submit-url`, `--submit-header`, and `--output json` with a structured `CommitTxResult` response.
...
Browse available transaction endpoints:

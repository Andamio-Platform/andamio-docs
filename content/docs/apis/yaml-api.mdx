...
Both `/api/transactions/{role}/{transaction}` and `/api/transaction` support:
...
# Get transaction with mainnet deployment resolution
curl "https://docs.andamio.io/api/transactions/general/mint-access-token?deployment=mainnet&version=v1"

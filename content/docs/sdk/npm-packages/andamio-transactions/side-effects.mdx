...
```typescript
{
  ...
  pathParams: {
    courseNftPolicyId: "buildInputs.policy",      // From transaction inputs
    moduleCode: "buildInputs.moduleCode",         // From side effect params
  }
}
```

At execution time, the path is resolved:
```
/course-modules/abc123.../MODULE_1/status
```
...
body: {
  ...
  courseNftPolicyId: { source: "context", path: "buildInputs.policy" }
}
```
...
// Definition (simplified)
{
  onSubmit: [{
    ...
    body: {
      courseNftPolicyId: { source: "context", path: "buildInputs.policy" },
      moduleCodes: { source: "context", path: "buildInputs.moduleCodes" },
  ...
  onConfirmation: [{
    ...
    body: {
      ...
      moduleCodes: { source: "context", path: "buildInputs.moduleCodes" },
      txHash: { source: "context", path: "txHash" },
      moduleHashes: { source: "context", path: "buildInputs.moduleHashes" },
    }
  }]
}
```
...
onSubmit: [{
  ...
  body: {
    status: {
      source: "conditional",
      condition: "buildInputs.assessmentResult",
      mapping: {
        "accept": "PENDING_TX_ASSIGNMENT_ACCEPTED",
        "deny": "PENDING_TX_ASSIGNMENT_DENIED"
      }
    }
  }
}]

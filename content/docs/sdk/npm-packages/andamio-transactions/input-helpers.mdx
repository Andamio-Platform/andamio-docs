...
const inputs = {
  ...
  module_infos: formatModuleInfos(modules),  // Helper formats data
};
...
```typescript
import { formatModuleInfos } from "@andamio/transactions";
import type { ListCourseModulesOutput } from "@andamio-platform/db-api";
...
const modules: ListCourseModulesOutput = [
  {
    ...
    title: "Introduction",
    slts: [
      { id: "1", text: "I can mint an access token." },
      { id: "2", text: "I can complete an assignment." }
    ],
    // ... other fields
  }
];

const moduleInfosJson = formatModuleInfos(modules);
// Returns JSON string for transaction API
```
...
### formatModuleInfosForMintModuleTokens (V1)
...
// Same signature as formatModuleInfos

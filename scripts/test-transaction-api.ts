...
async function testTransactionAPI() {
  ...
  const testCases = [
    {
      name: "General transaction (access-token-mint)",
      url: "/api/transaction?file=general/access-token-mint.yaml&deployment=preprod&version=v1",
    },
    {
    ...
    },
    {
      ...
      url: "/api/transactions/general/access-token-mint?deployment=preprod&version=v1",
    },
  ];
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (
        data.resolved?.addresses &&
        Object.keys(data.resolved.addresses).length > 0
      ) {
        console.log("\nResolved Addresses:");
      ...
      if (
        ...
        Object.keys(data.resolved.tokens).length > 0
      ) {
        console.log("\nResolved Tokens:");

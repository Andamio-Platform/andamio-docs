...
export async function GET(request: NextRequest) {
  try {
    ...
    if (role) {
      // Get transactions for specific role
      transactions = getAllTransactions("", role);
    } else {
    ...
    const transactionList = transactions.map(filePath => {
      ...
      const roleDir = parts.length > 1 ? parts[0] : 'general';
      ...
      return {
        ...
        url: `/api/transactions/${roleDir}/${transactionName}`
      };
    });

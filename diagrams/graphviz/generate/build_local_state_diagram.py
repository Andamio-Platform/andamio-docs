...
class LocalStateDiagramBuilder:
    ...
    def generate_validator_nodes(self, validators):
        ...
        if not validators:
            return nodes, connections, validator_ids
    ...
    def generate_transaction_nodes(self, transactions):
        ...
        if not transactions:
            return nodes, connections, transaction_ids
    ...
    def generate_action_nodes(self, config):
        ...
        return user_action_nodes, admin_action_nodes, connections, user_action_ids, admin_action_ids
    ...
    def build_diagram(self, config):
        ...
        validator_nodes, validator_connections, validator_ids = self.generate_validator_nodes(validators)
        lines.extend(validator_nodes)
        lines.append('')
        
        # Generate transaction nodes
        lines.append("    // Transaction nodes")
        transaction_nodes, transaction_connections, transaction_ids = self.generate_transaction_nodes(transactions)
        lines.extend(transaction_nodes)
        lines.append('')
        
        # Generate action nodes
        lines.append("    // Action nodes")
        user_action_nodes, admin_action_nodes, action_connections, user_action_ids, admin_action_ids = self.generate_action_nodes(local_state_def)
        ...
        # Connect roles to actions
        if user_action_ids:
            lines.append(f'    user_role -> {user_action_ids[0]} [arrowsize=0.6, penwidth=1, color="#4CAF50"];')
        
        if admin_action_ids:
            lines.append(f'    admin_role -> {admin_action_ids[0]} [arrowsize=0.6, penwidth=1, color="#4CAF50"];')
        
        # Connect actions to validators
        if user_action_ids and validator_ids:
            lines.append(f'    {user_action_ids[0]} -> {validator_ids[0]} [arrowsize=0.8, penwidth=1.5, color="#cc7000"];')
        
        if admin_action_ids and validator_ids:
            if len(validator_ids) > 1:
                lines.append(f'    {admin_action_ids[0]} -> {validator_ids[1]} [arrowsize=0.8, penwidth=1.5, color="#cc7000"];')
            else:
                lines.append(f'    {admin_action_ids[0]} -> {validator_ids[0]} [arrowsize=0.8, penwidth=1.5, color="#cc7000"];')
        ...
        if validator_ids and transaction_ids:
            for i, validator_id in enumerate(validator_ids):
                if i < len(transaction_ids):
                    lines.append(f'    {validator_id} -> {transaction_ids[i]} [arrowsize=0.6, penwidth=1, color="#2196F3"];')
                elif transaction_ids:
                    lines.append(f'    {validator_id} -> {transaction_ids[0]} [arrowsize=0.6, penwidth=1, color="#2196F3"];')
        
        # Add invisible edges to enforce column ordering
        if role_nodes and user_action_ids:
            lines.append(f'    admin_role -> {user_action_ids[0]} [style=invis, weight=10];')
        
        if user_action_ids and validator_ids:
            lines.append(f'    {user_action_ids[0]} -> {validator_ids[0]} [style=invis, weight=10];')

...
# Script to generate .dot and .png files from YAML local state diagram configuration

# Check if argument is provided
if [ $# -eq 0 ]; then
    echo "Usage: $0 <yaml_file>"
    echo "Example: $0 course-local-state.yaml"
    exit 1
fi
...
echo "Generating local state diagram for: $YAML_FILE"

# Step 1: Generate .dot file using the Python script
echo "Creating $OUTPUT_DIR/$DOT_FILE..."
if ! python3 build_local_state_diagram.py "$1" --yaml-dir="$YAML_DIR" --output-dir="$OUTPUT_DIR" -o "$DOT_FILE"; then
